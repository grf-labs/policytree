...
std::unique_ptr<Node> level_one_learning(const std::vector<flat_set>& sorted_sets,
  ...
  for (size_t p = 0; p < num_features; p++) {
    ...
    for (;;) {
      ...
      for (size_t d = 0; d < num_rewards; d++) {
        compute_level_one_reward(left_reward, right_reward, sum_array1, sum_array2, n, d, num_points, reward_type);
 ...
 * @param data: The data class
 * @param sum_array1 and sum_array2: A global zero initialized (num_rewards) x (num_points + 1)
